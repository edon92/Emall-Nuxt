...
import GoodModel from '../dbs/models/goods'
import CategorySubModel from '../dbs/models/CategorySub'
...
router.get('/getDetail', async(ctx, next) => {
  ...
  let res = await GoodModel.findOne({ID: goodId})
  ctx.body={
    code: 0,
    good: res
  }
...
router.get('/getCategoryListById', async(ctx, next) => {
  try {
    let categoryId = ctx.request.query.categoryId
      let result = await CategoryModel.find({ID: categoryId})     
    ctx.body= {
      code: 0 ,
      CategoryList: result
    }
...
//用category的ID，传入到用categorySub里面的MALL_CATEGORY_ID,可以找到一类
// 用categorySub里面的ID,传入到good的SUB_ID里面，可以周到一类商品
...
router.get('/getCategorySubList', async(ctx, next) => {
  try {
    let categorySubId = ctx.request.query.categorySubId
    let result = await CategorySubModel.find({MALL_CATEGORY_ID: categorySubId})
    ctx.body= {
      code: 0 ,
      CategorySubList: result
    }
  }catch(error) {
    ctx.body = {
      code: 500 ,
...
// router.get('/getGoodsListMaxPage', async(ctx, next) => {
//   try {
//     let goodsListId = ctx.request.query.goodsListId
//     let result = await GoodModel.find({SUB_ID: goodsListId}).exec()
...
router.get('/getGoodsList2', async(ctx, next) => {
  try {
    let goodsListId = ctx.request.query.goodsListId
    let page = ctx.request.query.page
    let num = 10
    let start = (page-1)*num
    let totalCount = await GoodModel.find({SUB_ID: goodsListId}).exec()
    let result = await GoodModel.find({SUB_ID: goodsListId}).skip(start).limit(10).exec()
    totalCount = Math.ceil(totalCount.length / 10)
    // let goodsListId = ctx.request.query.goodsListId
    // let result = await GoodModel.find({SUB_ID: goodsListId}).exec()
    ctx.body= {
      code: 0 ,
